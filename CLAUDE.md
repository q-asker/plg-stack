...
| **메트릭 저장소** | Prometheus | v3.13.1 | 시계열 메트릭 저장 및 PromQL 쿼리 |
...
MySQL 백업은 `git pull`만으론 안 되고 배포 사본을 갱신해야 한다:
```bash
cd ~/plg-stack && git pull origin main
sudo install -m 755 oci-mysql-backup/backup.sh /opt/oci-mysql-backup/backup.sh   # 바뀐 스크립트만
# lib까지 바뀌었으면 oci-mysql-backup/deploy.sh 재실행
```
...
`q-asker/`는 **모노레포가 아니다** — git이 아닌 작업 폴더이며, 그 아래 **독립 git repo**(plg-stack·api·client)와
**git 아닌 로컬 디렉토리**(infra·specs·android)가 섞여 있다. plg-stack이 의존·참조하는 이웃을 명시한다
(경로는 plg-stack 기준 `../<name>`).

| 디렉토리 | git | 정체 / plg-stack과의 관계 |
|----------|-----|--------------------------|
| `.` (plg-stack) | **독립 repo** | **이 레포.** PLG 모니터링 스택 + 백업·복구 스크립트. IAM·버킷을 *소비*만 함 |
| `../infra/oci/` | 로컬(비-git) | **백업 버킷·IAM의 진실의 원천** — OCI Terraform(로컬 state, profile=DEFAULT). `qasker-mysql-backup`·`qasker-monitoring-backup` 버킷·lifecycle + `.env`가 참조하는 IAM 프로필(BACKUP_WRITER/READER, BACKUP_MON_WRITER/READER, BACKUP_USAGE_READER)의 group·user·policy 정의. **버전관리 안 됨 주의** |
| `../infra/gcp/` | 로컬(비-git) | GCP Terraform (plg-stack 직접 관여 없음) |
| `../api/` | **독립 repo** | **q-asker Spring Boot 앱**(Java/Gradle). plg-stack이 관측·백업하는 OCI-2 애플리케이션. `api/modules/oci`에도 OCI 코드 존재 |
| `../client/` | **독립 repo** | 프론트엔드(npm) |
| `../android/` | 로컬(비-git) | 안드로이드 앱(Gradle) |
| `../specs/` | 로컬(비-git) | Spec Kit 제품 스펙(퀴즈 기능 등). 백업 스펙(`001-prometheus-loki-backup-recovery` 등)은 여기 아님 — 로컬/gitignored |
| `../api-wt-feedback7/`, `../client-wt-feedback7/` | worktree | api·client의 git worktree(feedback7). 건드리지 않음 |
| `../CLAUDE.md` | — | 작업 폴더 루트 지침(이슈→스펙→하네스 파이프라인) |

**주의**: IAM 프로필·버킷·lifecycle을 코드가 아닌 곳에서 바꿔야 하면 `../infra/oci/`의 Terraform이다
(`object_storage.tf`=MySQL 버킷·IAM, `object_storage_monitoring.tf`=PLG 버킷·IAM,
`object_storage_usage_reader.tf`=총량 조회 프로필). plg-stack 안의 스크립트는 이 IAM을 **소비**만 한다.

## 작업 범위 제한

- **Kafka 브로커 설정은 이 프로젝트에서 관리하지 않는다** — broker-1/2/3은 별도 프로젝트로 분리됨
- **버킷·IAM 정의는 이 프로젝트가 아니라 `../infra/oci/` Terraform에서 관리한다** — plg-stack은 소비자
