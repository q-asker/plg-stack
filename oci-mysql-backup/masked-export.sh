...
#                      [--upload] [--bucket=qasker-mysql-backup] [--oci-profile=BACKUP_WRITER]
#                      [--check-only] [--dry-run]
#   --check-only : 커버리지 가드만(미분류 컬럼 있으면 실패). CI 용.
#   --dry-run    : 생성될 마스킹 SQL 미리보기(적용·덤프 안 함).
#   --upload     : 마스킹 덤프+sha256 을 OCI 버킷 masked/ prefix 에 업로드
...
if [ "$UPLOAD" = 1 ]; then
  ...
  log "   개발자는 restore-local.sh 로 이 파일(마스킹본)만 반입. (--upload 로 OCI 업로드 가능)"
fi
