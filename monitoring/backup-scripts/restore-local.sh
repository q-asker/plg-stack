...
#   옵션: --bucket=NAME --profile=READER --strip=1 --dry-run -h
#
# 흐름:
#   1. tar 확보(--file 그대로 / 아니면 OCI download_object)
...
#
# 종료 코드: 0 성공 / 2 인자오류 / 4 다운로드실패 / 10 not ready / 12 docker오류
set -euo pipefail
...
# ─── 인자 ───
FILE="" SNAPSHOT="" LATEST=0 DRY_RUN=0 STRIP=1
...
if (( DRY_RUN )); then
  log INFO "[dry-run] 다운로드까지만. 볼륨 변경 없음."
  exit 0
fi
